...
public class GreetService implements Service {
	...
	GreetService(Config config) {
		this.greeting = config.get(CONFIG_KEY_GREETING).asString().orElse(DEFAULT_GREETING);
		this.greetingSupplier = config.get(CONFIG_KEY_GREETING).asString().supplier(DEFAULT_GREETING);
		...
		this.defaultMessageCounter = metricRegistry.counter("greet.default.counter");
		this.messageCounter = metricRegistry.counter("greet.message.counter");
		this.updateMessageCounter = metricRegistry.counter("greet.message.update.counter");
	}
	...
	public void update(Routing.Rules rules) {
		rules.get("/", this::getDefaultMessageHandler).get("/{name}", this::getMessageHandler).put("/greeting",
	...
	private void getDefaultMessageHandler(ServerRequest request, ServerResponse response) {
		...
		String user = request.context().get(SecurityContext.class).map(SecurityContext::userName).orElse("World");

		sendResponse(response, user);
		this.defaultMessageCounter.inc();
	}
	...
	private void getMessageHandler(ServerRequest request, ServerResponse response) {
		...
		String user = request.context().get(SecurityContext.class).map(SecurityContext::userName).orElse("Anonymous");
